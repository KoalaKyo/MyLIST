...
type BootstrapData = { deviceId: string; theme: Theme; locale: Locale; startupEnabled: boolean; categories: Category[]; palette: Array<{ id: string; row: number; column: number; value: string }> };
type Task = { id: string; title: string; note: string; categoryId: string; categoryName: string; categoryDefaultKey: DefaultCategoryKey | null; categoryNameOverride: string | null; categoryColor: string; status: Status; dueAtUtcMs: number | null; createdAtUtcMs: number; updatedAtUtcMs: number; completedAtUtcMs: number | null };
...
export default function App() {
  ...
  function showError(error: unknown) {
    ...
    showNotice(/unique constraint failed: categories\.name|已存在同名分类/i.test(message) ? t("notice.sameCategory") : t("error.operationFailed"));
  }
  ...
  async function previewEncryptedImport(sessionId: string, password: string, operation: ImportOperation) { try { const preview = await invoke<ImportPreview>("preview_pending_encrypted_import", { sessionId, password }); setImportOperation(operation); setEncryptedImport(null); setImportPreview(preview); return true; } catch (error) { showError(error); return false; } }
  async function applyPlaintextImport(sessionId: string, operation: ImportOperation) { try { const result = await invoke<ImportResult>("apply_pending_plaintext_import", { sessionId, operation }); await Promise.all([refreshBootstrap(), refreshTasks()]); setImportPreview(null); showNotice(operation === "replace" ? t("data.importCompleteReplace", { count: result.newTasks }) : t("data.importCompleteMerge", { newCount: result.newTasks, updatedCount: result.updatedTasks })); return true; } catch (error) { showError(error); return false; } }
  ...
  return <main className="app-shell" data-theme={theme} lang={locale}>
    {page === "home" ? <>
      ...
      <section className="task-page">
        ...
        <TaskList>
          ...
          {!visibleTasks.length && <div className="empty-state"><img src={icon("tag_20_regular.svg")} alt="" /><p>{status === "todo" ? t("task.emptyTodo") : t("task.emptyCompleted")}</p><span>{status === "todo" ? t("task.emptyTodoHelp") : t("task.emptyCompletedHelp")}</span></div>}
        </TaskList>
      </section>
    ...
    </> : page === "settings" ? <Settings theme={theme} locale={locale} startupEnabled={bootstrap?.startupEnabled ?? true} categories={categories} palette={bootstrap?.palette ?? []} categoryUsage={Object.fromEntries(categories.map((category) => [category.id, tasks.todo.filter((task) => task.categoryId === category.id).length + tasks.completed.filter((task) => task.categoryId === category.id).length]))} onThemeChange={setThemeSetting} onLocaleChange={setLocaleSetting} onStartupChange={setStartupSetting} onCreateCategory={createCategory} onUpdateCategory={updateCategory} onDeleteCategory={deleteCategory} onRestoreDefaults={restoreDefaultCategories} onOpenExport={() => setExportDialogOpen(true)} onPreviewImport={previewPlaintextImport} onBack={() => setPage("home")} /> : page === "view" && selectedTask ? <TaskView task={selectedTask} onBack={() => setPage("home")} onEdit={() => setPage("edit")} onDelete={() => void removeTask(selectedTask)} onCopy={(text, successNotice) => void copyText(text, successNotice)} /> : <TaskForm title={formMode} task={page === "edit" ? selectedTask : null} categories={categories} theme={theme} onBack={() => setPage(selectedTask ? "view" : "home")} onSave={async (input) => { try { const editing = page === "edit" && selectedTask; await (editing ? invoke<Task>("update_task", { input: { ...input, id: selectedTask.id } }) : invoke<Task>("create_task", { input })); await refreshTasks(); setSelectedTask(null); setStatus("todo"); setPage("home"); showNotice(editing ? t("notice.taskSaved") : t("notice.taskAdded")); } catch (error) { showError(error); } }} />}
    {notice && <div className="toast" role="status">{notice}</div>}
    {importPreview && <ImportPreviewDialog preview={importPreview} operation={importOperation} theme={theme} onApply={applyPlaintextImport} onClose={() => setImportPreview(null)} />}
    {encryptedImport && <EncryptedImportPasswordDialog theme={theme} request={encryptedImport} onClose={() => setEncryptedImport(null)} onPreview={previewEncryptedImport} />}
    {exportDialogOpen && <ExportDataDialog theme={theme} onClose={() => setExportDialogOpen(false)} onExportPlaintext={exportPlaintextData} onExportEncrypted={exportEncryptedData} />}
...
function Header({ mode, onCycle, onHide }: { mode: WindowMode; onCycle: () => void; onHide: () => void }) {
  return <header className="app-titlebar" onMouseDown={startWindowDrag} onDoubleClick={preventTitlebarDoubleClick}><button className={`icon-control pin pin-${mode.replace("mode-", "")} ${mode === "mode-topmost" ? "is-active" : ""}`} aria-label={t("app.windowMode")} onClick={onCycle}><img src={icon("pin_24_regular.svg")} alt="" /></button><h1 className="wordmark-title"><span className="wordmark-mark" role="img" aria-label="MyLIST" style={{ "--wordmark-image": `url("${wordmarkLogo}")` } as CSSProperties} /></h1><button className="icon-control close" aria-label={t("app.hideToTray")} onClick={onHide}><img src={icon("dismiss_20_regular.svg")} alt="" /></button></header>;
}

function Settings({ theme, locale, startupEnabled, categories, palette, categoryUsage, onThemeChange, onLocaleChange, onStartupChange, onCreateCategory, onUpdateCategory, onDeleteCategory, onRestoreDefaults, onOpenExport, onPreviewImport, onBack }: { theme: Theme; locale: Locale; startupEnabled: boolean; categories: Category[]; palette: BootstrapData["palette"]; categoryUsage: Record<string, number>; onThemeChange: (theme: Theme) => void; onLocaleChange: (locale: Locale) => void; onStartupChange: (enabled: boolean) => void; onCreateCategory: () => void; onUpdateCategory: (input: { id: string; name: string; colorId: string }) => Promise<boolean>; onDeleteCategory: (id: string, targetCategoryId?: string) => Promise<boolean>; onRestoreDefaults: () => Promise<boolean>; onOpenExport: () => void; onPreviewImport: (operation: ImportOperation) => Promise<void>; onBack: () => void }) {
  ...
  return <section className="sheet settings-sheet">
    ...
    <nav className="mode-tabs settings-tabs" role="tablist" aria-label={t("settings.tabs")}>
      <button className={section === "general" ? "selected" : ""} onClick={() => setSection("general")}>{t("settings.general")}</button>
      <button className={section === "categories" ? "selected" : ""} onClick={() => setSection("categories")}>{t("settings.categories")}</button>
      <button className={section === "data" ? "selected" : ""} onClick={() => setSection("data")}>{t("settings.data")}</button>
    </nav>
...
function ExportDataDialog({ theme, onClose, onExportPlaintext, onExportEncrypted }: { theme: Theme; onClose: () => void; onExportPlaintext: () => Promise<boolean>; onExportEncrypted: (password: string) => Promise<boolean> }) {
...
function LanguageDropdown({ value, theme, onChange }: { value: Locale; theme: Theme; onChange: (locale: Locale) => void }) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const rect = triggerRef.current!.getBoundingClientRect();
      const padding = 8;
      const desired = Math.min(224, localeOptions.length * 32 + 8);
...
function TaskRow({ task, theme, onOpen, onEdit, onCopy, onStatus, onDelete }: { task: Task; theme: Theme; onOpen: () => void; onEdit: () => void; onCopy: () => void; onStatus: () => Promise<boolean>; onDelete: () => void }) {
  ...
  return <article className={`task-row task-row-${task.status} ${confirming === "status" ? "status-confirming" : ""} ${confirming === "delete" ? "delete-confirming" : ""} ${collapsing === "status" ? "status-collapsing" : ""} ${collapsing === "delete" ? "delete-collapsing" : ""} ${statusFading ? "status-fading" : ""} ${deleteFading ? "delete-fading" : ""} ${statusExiting ? `status-exiting status-exiting-${completed ? "left" : "right"}` : ""}`} style={{ "--delete-confirm-width": `${deleteConfirmWidth}px` } as CSSProperties} onMouseLeave={() => { setTooltip(null); if (statusExiting) return; if (transitionLocked) setCollapseAfterTransition(true); else closeConfirmation(); }}>
    ...
    <span className={`task-time${task.dueAtUtcMs !== null && task.dueAtUtcMs < Date.now() ? " overdue" : ""}`} aria-label={task.dueAtUtcMs ? t("calendar.due") : undefined}>{formatTaskTime(task.dueAtUtcMs)}</span>
    {completed ? <button ref={deleteRef} className="task-delete" aria-label={confirming === "delete" ? t("task.confirmDelete") : t("task.deleteTask")} aria-disabled={transitionLocked} onClick={handleDelete}><img className="delete-base-icon" src={icon("delete_20_regular.svg")} alt="" /><img className="delete-overlay-icon" src={icon("delete_20_regular.svg")} alt="" /><span ref={deleteLabelRef}>{t("task.delete")}</span></button> : <><button ref={menuRef} className={`task-menu-trigger ${menuOpen ? "open" : ""}`} aria-label={t("task.more")} aria-expanded={menuOpen} onClick={handleMenuToggle}><span>•••</span></button>{menuOpen && <TaskMenu anchor={menuRef.current} theme={theme} onEdit={() => { setMenuOpen(false); onEdit(); }} onCopy={() => { setMenuOpen(false); onCopy(); }} onDelete={() => { setMenuOpen(false); onDelete(); }} onDismiss={() => setMenuOpen(false)} />}</>}
...
function TaskForm({ title, task, categories, theme, onBack, onSave }: { title: string; task: Task | null; categories: Category[]; theme: Theme; onBack: () => void; onSave: (input: { title: string; note: string; categoryId: string; dueAtUtcMs: number | null }) => void }) {
  const [draft, setDraft] = useState(() => ({ title: task?.title ?? "", note: task?.note ?? "", categoryId: task?.categoryId ?? categories[0]?.id ?? "", dueAtUtcMs: task?.dueAtUtcMs ?? null }));
  useEffect(() => setDraft({ title: task?.title ?? "", note: task?.note ?? "", categoryId: task?.categoryId ?? categories[0]?.id ?? "", dueAtUtcMs: task?.dueAtUtcMs ?? null }), [task, categories]);
  function submit(event: FormEvent) { event.preventDefault(); onSave(draft); }
  return <section className="sheet"><SheetHeader title={title} onBack={onBack} /><form className="task-form" onSubmit={submit}><label>{t("form.title")}<input autoFocus value={draft.title} maxLength={200} placeholder={t("form.titlePlaceholder")} onChange={(event) => setDraft({ ...draft, title: event.target.value })} /></label><label>{t("form.note")}<textarea value={draft.note} maxLength={2000} placeholder={t("form.notePlaceholder")} onChange={(event) => setDraft({ ...draft, note: event.target.value })} /></label><div className="task-form-pair"><label>{t("form.type")}<CategoryDropdown value={draft.categoryId} categories={categories} theme={theme} onChange={(categoryId) => setDraft({ ...draft, categoryId })} /></label><label>{t("form.due")}<DateTimePicker value={draft.dueAtUtcMs} theme={theme} onChange={(dueAtUtcMs) => setDraft({ ...draft, dueAtUtcMs })} /></label></div><p className="form-help">{t("form.help")}</p><footer className="sheet-footer"><button type="button" className="button secondary" onClick={onBack}>{t("app.back")}</button><button type="submit" className="button primary">{t("task.save")}</button></footer></form></section>;
}
...
function ViewDeleteButton({ onDelete }: { onDelete: () => void }) {
  ...
  return <button type="button" className={`view-delete-confirm ${state}`} style={{ "--view-delete-width": `${width}px` } as CSSProperties} aria-label={state === "expanded" ? t("task.confirmDelete") : t("task.deleteTask")} onClick={activate} onMouseLeave={collapse}><img className="delete-base-icon" src={icon("delete_20_regular.svg")} alt="" /><img className="delete-overlay-icon" src={icon("delete_20_regular.svg")} alt="" /><span ref={labelRef}>{t("task.delete")}</span></button>;
}
